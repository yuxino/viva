enum TABS {
  ADD_NEW_TAB = 'TAB_ADD_NEW_TAB', // add a new tab to the tabs
  CLOSE_TAB = 'TAB_CLOSE_TAB', // close one tab
  CLOSE_ALL_TABS = 'TAB_CLOSE_ALL_TABS', // close all tab
  CLOSE_LEFT_TABS = 'TAB_CLOSE_LEFT_TABS', // close tabs on the left side of tab
  CLOSE_RIGHT_TABS = 'TAB_CLOSE_RIGHT_TABS', // close tabs on the right side of tab
  CLOSE_SAVED_TABS = 'TAB_CLOSE_SAVED_TABS', // close all saved state tab
  UPDATE_TAB_ORDER = 'TAB_UPDATE_TAB_ORDER' // update tab order
}

export default TABS;
